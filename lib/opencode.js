...
const HUB_VERSION = 5; // bump when hub shape/logic changes — old hubs on
  ...
  (async () => {
    for (;;) {
      ...
      try {
        ...
        // disposes after idle — the subscription then starves silently.
        // Cure: act() calls hub.resubscribe() before every run, so we always
        // attach to the instance that run wakes.
