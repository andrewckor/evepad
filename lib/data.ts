...
import { resolveProject, projectsError } from "./projects";
import { analyticsFor, readRunEvents, windowFor, type RunEvent } from "./vercel-client";
import type {
  Project,
  RawRun,
  RunSession,
...
export type ListRunsResult = {
  ...
  sessions: RunSession[];
  error: string | null;
  auth: AuthFailure | null;
};

export const ENVIRONMENTS = ["local", "preview", "production"] as const;

const dataDir = (path: string) => join(path, ".eve", ".workflow-data");
const readJson = (p: string) => JSON.parse(readFileSync(p, "utf8"));
const safeList = (d: string): string[] => (existsSync(d) ? readdirSync(d) : []);
const num = (v: unknown) => (v == null ? 0 : Number(v) || 0);

// --- local adapter --------------------------------------------------------

// Chunk files are devalue-wrapped base64 holding one NDJSON event.
function decodeChunk(file: string): RunEvent | null {
  const raw = readFileSync(file, "latin1");
  const m = raw.match(/"([A-Za-z0-9+/]{40,}={0,2})"/);
  if (!m) return null;
  try {
    return m[1] ? JSON.parse(Buffer.from(m[1], "base64").toString("utf8")) : null;
  } catch {
    return null;
  }
}
...
function streamNames(runId: string, project: string): string[] {
...
function localEvents(runId: string, project: string): RunEvent[] {
  const out: RunEvent[] = [];
...
function localRuns(project: string): RawRun[] {
  return safeList(join(dataDir(project), "runs"))
    .filter((f) => f.endsWith(".json"))
    .map((f) => {
      try {
        return readJson(join(dataDir(project), "runs", f));
      } catch {
        return null;
      }
    })
...
async function vercelRuns(
  project: Project,
  environment: string,
  limit: number,
  { periodMs, attributes }: { periodMs?: number; attributes?: Record<string, string> } = {},
): Promise<RawRun[]> {
...
async function vercelEvents(
  ...
  environment: string,
  runId: string,
): Promise<{ events: RunEvent[]; note: string | null }> {
  try {
    ...
    return {
      events: [],
      note: `Could not read the session stream: ${e instanceof Error ? e.message : e}`,
    };
  }
}

async function vercelRun(
  project: Project,
  environment: string,
  runId: string,
): Promise<RawRun | null> {
...
function toSessions(runs: RawRun[]): RunSession[] {
  ...
  return sessions
    .map((s) => {
      ...
      const roll = (k: string) => kids.reduce((sum, c) => sum + num(c.attributes?.[k]), 0);
      const end =
        s.completedAt ??
        kids
          .map((k) => k.completedAt)
      ...
      return {
        runId: s.runId,
        title: String(s.attributes?.["$eve.title"] ?? "(untitled)"),
        trigger: String(s.attributes?.["$eve.trigger"] ?? "—"),
        status: s.status,
        createdAt: s.createdAt,
        durationMs: end ? new Date(end).getTime() - new Date(s.createdAt).getTime() : null,
        model:
          (kids.find((k) => k.attributes?.["$eve.model"])?.attributes?.["$eve.model"] as
            | string
            | undefined) ?? null,
...
async function swrCache<T>(
  ...
  if (hit && age <= hit.ttl) return hit.data as T;
  if (hit && age <= staleTtl) {
    if (!inflight.has(key)) {
      inflight.set(
        key,
    ...
    return hit.data as T; // stale, but instant — the refresh lands for the next poll
  }
  if (inflight.has(key))
    return inflight.get(key)!.then(() => (cache.get(key)?.data as T | undefined) ?? fetcher());
  const p = fetcher()
    .then((data) => cacheSet(key, data, ttl))
    .finally(() => inflight.delete(key));
  inflight.set(key, p);
...
export async function listRuns({
  ...
  const errors: Array<{ env: string | null; message: string }> = [];
      ...
      try {
        ...
        if (env === "local") {
          if (!project.localPath)
            throw new Error("local: no checkout — start `eve dev` for this project");
        ...
        errors.push({ env, message: e instanceof Error ? e.message : String(e) });
        return [];
      }
    }),
  );
...
function authFailure(errors: Array<{ env: string | null; message: string }>): AuthFailure | null {
  for (const { env, message } of errors) {
    ...
    if (/upgrade-required|observability-upgrade|lookback/i.test(message)) {
      return { env: env as AuthFailure["env"], kind: "plan", canReconnect: false, message };
    }
    // No credential at all — same remedy as an expired one, so same offer.
    if (/no vercel cli token|vercel login/i.test(message)) {
      return { env: env as AuthFailure["env"], kind: "missing", canReconnect: true, message };
    }
    if (/\b401\b|unauthorized|invalid token|token.*(expired|revoked)/i.test(message)) {
      return { env: env as AuthFailure["env"], kind: "expired", canReconnect: true, message };
    }
    ...
    if (/\b403\b|forbidden/i.test(message)) {
      return { env: env as AuthFailure["env"], kind: "forbidden", canReconnect: true, message };
    }
  }
  return null;
}
...
function foldEvents(events: RunEvent[]): Turn[] {
  type FoldTurn = Omit<Turn, "steps" | "durationMs"> & { steps: Map<number, TurnStep> };
  const turns = new Map<string, FoldTurn>();
  const byCallId = new Map<string, ToolCall>();
  const turnOf = (id: string): FoldTurn => {
    if (!turns.has(id))
      turns.set(id, { turnId: id, steps: new Map(), messages: [], startedAt: null, endedAt: null });
    return turns.get(id)!;
  };
  const stepOf = (t: FoldTurn, i: number): TurnStep => {
    if (!t.steps.has(i))
      t.steps.set(i, {
        stepIndex: i,
        calls: [],
  ...
  for (const e of events) {
    ...
    const at: string | null = e.meta?.at ?? null;
    ...
    if (e.type === "actions.requested") {
      ...
      if (c) {
        ...
        c.endedAt = at;
        if (c.startedAt && at)
          c.durationMs = new Date(at).getTime() - new Date(c.startedAt).getTime();
      }
...
function extractText(d: EventData): string | null {
...
export async function getRun(
  ...
  if (environment === "local") {
    ...
    return {
      ...
      events: events.map((e) => ({ type: e.type, at: e.meta?.at ?? null, data: e.data })),
      note: null,
    };
  }

  const key = `run:${project.name}:${environment}:${runId}`;
  const cached = fresh ? undefined : (cacheGet(key) as RunDetail | null);
  ...
  const [run, recent, streamResult] = await Promise.all([
    ...
    vercelEvents(project, environment, runId).catch((e: unknown) => ({
      events: [] as RunEvent[],
      note: `Could not read the remote session stream: ${e instanceof Error ? e.message : e}`,
    })),
  ]);
  if (!run) return null;
