...
export async function vercelProjects() {
  ...
  try {
    ...
    lastProjectsError = String(e.message ?? e);
...
const eveVerCache = new Map(); // root -> {at, v}
export function eveVersionAt(root) {
  if (!root) return null;
  const hit = eveVerCache.get(root);
  if (hit && Date.now() - hit.at < 60_000) return hit.v;
  let v = null;
  try { v = JSON.parse(readFileSync(join(root, "node_modules", "eve", "package.json"), "utf8")).version ?? null; } catch {}
  if (!v) {
    try {
      const deps = JSON.parse(readFileSync(join(root, "package.json"), "utf8"));
      v = (deps.dependencies?.eve ?? deps.devDependencies?.eve ?? "").replace(/^[^0-9]*/, "") || null;
    } catch {}
  }
