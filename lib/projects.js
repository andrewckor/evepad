...
import { knownPath, remember } from "./registry.js";
...
export async function listProjects() {
  ...
  // Unfiltered: non-eve projects render too (framework drives the tile art);
  // eve projects and live servers sort first so agents stay the headline.
  const merged = remote.map((p) => {
    ...
    return {
      ...
      model: s?.model ?? null,
      // A live eve dev server proves the framework even when the Vercel
      // record has none (projects created before the framework tag existed).
      framework: p.framework ?? (s ? "eve" : null),
    };
  });
  ...
  merged.sort((a, b) =>
    ...
    Number(b.framework === "eve") - Number(a.framework === "eve") ||
