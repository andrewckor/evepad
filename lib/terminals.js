...
export async function startTerm(project, variant = "eve", size = {}) {
  ...
  if (variant === "login") {
    ...
    const term = makeTerm({
      name: "__login",
      // `vercel login` directly, not a shell that we then type into: the pty
      // is only created when the play button is pressed, so the command IS the
      // consent. Seeding a shell instead meant racing its line editor — the
      // box came up at a bare prompt often enough to be a bug.
