...
export default function ProjectLogo({ p, size = 32 }) {
  ...
  const mark = p.framework === "eve"
    ? <EveMark />
    : <span className="mono" style={{ fontSize: size < 24 ? 10 : 13 }}>{(p.name ?? "?").slice(0, 1).toUpperCase()}</span>;
  return (
    <span className="agentlogo" style={{ width: size, height: size, borderRadius: size < 24 ? 6 : 8 }}>
      {src && (
        <img
