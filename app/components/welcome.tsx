...
function Command({ children }: { children: string }) {
  ...
  return (
    <div className="wc-cmd">
      <span className="wc-cmd-ic">
        <Terminal />
      </span>
      <code className="mono">{children}</code>
      <button
        className={"wc-cmd-copy" + (copied ? " done" : "")}
        onClick={copy}
...
export function CliSignIn({
    ...
    return (
      <>
        <div className="wc-found">
          <span className="wc-found-ic">
            <CheckCircleFill />
          </span>
          <span className="wc-found-text">
            <b>{found.user?.name}</b>
            <i>
              {found.user?.email} · {found.scope?.name}
            </i>
          </span>
        </div>
  ...
  if ((latest ?? account)?.tokenSource === "VERCEL_TOKEN") {
    return (
      <>
        <p className="wc-body">
          evepad is using <span className="mono">VERCEL_TOKEN</span> from its environment, and
          Vercel rejected it.
        </p>
        <p className="wc-foot">
          Replace that token and restart evepad. Signing in with the CLI won&rsquo;t help while
          it&rsquo;s set — the variable takes priority.
        </p>
      </>
    );
  }
...
export default function Welcome({
  ...
  return (
    <div className="wc">
      ...
      <p className="wc-body">
        This scope has no eve agents. Create one and evepad scaffolds it, makes its Vercel project,
        and starts it locally — then Build, Runs and the terminal all point at it.
      </p>
      <div className="wc-actions">
        <Button onClick={onNew}>
          Create your first agent <ArrowRight />
        </Button>
      </div>
      <p className="wc-foot">
        Already have one on this Mac? Run <span className="mono">eve dev</span> in its folder once
