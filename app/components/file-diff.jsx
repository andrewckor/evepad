...
const kindOf = (line) => (line[0] === "+" ? "add" : line[0] === "-" ? "del" : "ctx");
      ...
      {hunks.map((h, i) => {
        ...
        return (
          <div key={i} className="fd-hunk">
            <div className="fd-hunk-head">@@ -{h.oldStart},{h.oldLines} +{h.newStart},{h.newLines} @@</div>
            {h.lines.map((line, j) => {
...
export default function FileDiff({ project, file, additions, deletions, defaultOpen = false }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/oc/file?project=${encodeURIComponent(project)}&path=${encodeURIComponent(file)}`)
      .then((r) => r.json())
      .then((d) => { if (!cancelled) d.error ? setError(d.error) : (setNote(d.note ?? null), setHunks(d.hunks ?? [])); })
      .catch(() => { if (!cancelled) setError("Could not read the file."); });
  ...
  return (
    <div className={"fd" + (open ? " open" : "")}>
      <button className="fd-head" onClick={() => setOpen((o) => !o)}>
        <span className="fd-chev"><ChevronRight /></span>
        <span className="fd-file mono" title={file}>{file.split("/").slice(-2).join("/")}</span>
        {additions != null && <span className="fd-add">+{additions}</span>}
        {deletions != null && <span className="fd-del">−{deletions}</span>}
      </button>
      {open && (
        error ? <div className="fd-empty">{error}</div>
        : note ? <div className="fd-empty">{note}</div>
