...
import type { OcPart } from "./oc-types";
import {
  Sparkles,
  ChevronDownSmall,
  Terminal,
...
function headline(parts: OcPart[], busy: boolean): string {
...
export default function Thinking({ parts, busy }: { parts: OcPart[]; busy: boolean }) {
        ...
        <div className="th-clip">
          <div className="th-trace">
            {parts.map((p) => {
              ...
              const raw = p.tool === "webfetch" ? p.state?.input?.url : null;
