  ...
  return (
    <>
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverTrigger className="acc-chev" title={data?.loggedIn ? `${label} — Vercel account` : "Vercel account"}>
          ...
          {data?.loggedIn ? (
            <>
              ...
              {[scope, ...(data.teams ?? []).filter((t) => t.id !== scope?.id)].filter(Boolean).map((t) => (
                <div key={t.id ?? t.slug} className={"menu-row acc-team" + (t.slug === scope?.slug ? " on" : "")}>
