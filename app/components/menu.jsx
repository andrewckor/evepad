...
export function useScrollFade(enabled = true, selector) {
  ...
  useEffect(() => {
    ...
    let el = selector ? document.querySelector(selector) : ref.current;
    let raf;
    if (!el && selector) {
    ...
    function attach(node) {
      const update = () => {
        ...
        const next = room <= 1 ? "none"
          : node.scrollTop <= 1 ? "start"
          : node.scrollTop >= room - 1 ? "end"
          : "middle";
      ...
      cleanup = () => { node.removeEventListener("scroll", update); ro.disconnect(); mo.disconnect(); };
    }

    if (el) attach(el);
    return () => { if (raf) cancelAnimationFrame(raf); cleanup?.(); };
  }, [enabled, selector]);
...
export function MenuList({ scroll = false, max = 320, className = "", children }) {
...
export function MenuRow({ on = false, onSelect, trail, className = "", children, render }) {
  const inner = (
    <>
      <span className="menu-row-label">{children}</span>
      {trail !== undefined && <span className="menu-row-trail">{trail}</span>}
      {on && <span className="menu-check"><Check /></span>}
    </>
  );
