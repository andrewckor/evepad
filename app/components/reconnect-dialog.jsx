...
import { CliSignIn } from "./welcome.jsx";
...
export default function ReconnectDialog({ open, onOpenChange, kind = "forbidden", onReconnected }) {
  return (
      ...
      <DialogContent className="set-dialog">
        ...
        <div className="set-body">
          <DialogHeader>
            ...
            <DialogDescription>{COPY[kind] ?? COPY.forbidden}</DialogDescription>
          </DialogHeader>
          ...
          <div className="rc-signin">
            <CliSignIn
              terminal
              onContinue={() => { onReconnected?.(); onOpenChange(false); }}
            />
          </div>
        </div>
      </DialogContent>
