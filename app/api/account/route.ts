...
import { cliToken, currentTeam } from "@/lib/projects";
...
let cache: { at: number; key: string | null; data: unknown } = { at: 0, key: null, data: null };
const TTL = 60_000;
...
export async function GET() {
  ...
  try {
    ...
    const u = me.user ?? me;
    const teams = ((teamList.teams ?? []) as VercelTeam[]).map((t) => ({
      id: t.id,
      slug: t.slug,
      name: t.name ?? t.slug,
