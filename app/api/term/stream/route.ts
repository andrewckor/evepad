...
export async function GET(request: Request) {
  let getTerm;
  try {
    ({ getTerm } = await terminals());
  } catch {
    return new Response("terminals unavailable on this platform", { status: 501 });
  }
  ...
  const term = getTerm(
    name,
    (url.searchParams.get("variant") as import("../../../../lib/terminals").TermVariant | null) ??
      undefined,
  );
  ...
  const stream = new ReadableStream({
    ...
    cancel() {
      if (ctrl) term.subscribers.delete(ctrl);
    },
  });
