...
function parseHunks(patch: string): Hunk[] {
  ...
  for (const line of patch.split("\n")) {
    ...
    if (header) {
      current = {
        oldStart: +header[1]!,
        oldLines: header[2] ? +header[2] : 1,
        newStart: +header[3]!,
        newLines: header[4] ? +header[4] : 1,
        lines: [],
      };
      hunks.push(current);
      continue;
...
export async function GET(request: Request) {
  ...
  if (file.startsWith(".."))
    return Response.json({ error: "Outside this agent's folder." }, { status: 400 });

  try {
    // HEAD-relative so a staged-but-uncommitted edit still shows; --no-color
    // because the renderer draws its own.
    const { stdout } = await exec("git", ["diff", "HEAD", "--no-color", "-U3", "--", file], {
    ...
    if (
      /not a git repository|unknown revision|ambiguous argument|could not access/i.test(
